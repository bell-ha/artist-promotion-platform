...
export default function AuthModals({ isOpen, mode, onClose, onModeChange, onLoginSuccess }: AuthModalsProps) {
  ...
  const handleGoogleSuccess = async (credentialResponse: CredentialResponse) => {
    try {
      ...
      const { access_token, is_new_user, nickname, email, user_id, role } = res.data as {
        access_token: string;
        is_new_user: boolean;
        nickname: string;
        email: string;
  ...
  const handleSignIn = async () => {
    try {
      ...
      const data = res.data as { access_token: string; nickname: string; user_id: number; role: string };
